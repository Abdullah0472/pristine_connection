...
// Convert each image to base64 and add to the string
      ...
      // });
      Map<String, dynamic> response = await _api.signUpApi(data);

      if (response != null && response['status_code'] == 200) {
        Utils.snackBar('Account Created', 'Successfully');
        print('The Data is Upload Successfully');

      } else {
        Utils.snackBar(
            'Error', response['error'] ?? 'An error occurred while creating account');
      }
    } catch (e) {
      Utils.snackBar('Account Created', 'Successfully');
