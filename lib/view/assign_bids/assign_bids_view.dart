...
class _AssignBidViewState extends State<AssignBidView> {
  ...
  final startTripVM = Get.put(StartTripController());
        ...
        switch (assignBidsVM.rxRequestStatus.value) {
          ...
          case Status.COMPLETED:
            return (assignBidsVM.assignbidsList.value.data!.isNotEmpty)
                ? SmartRefresher(
              controller: assignBidsVM.refreshAssignBidController,
              onRefresh: () async {
                await  assignBidsVM.refreshApi();
                assignBidsVM.refreshAssignBidController.refreshCompleted();
              },
                  child: SizedBox(
              height:
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      ...
                      return BidCard(
                        ...
                        hazardous: assignBidsVM.assignbidsList.value.data![index].hazardous
                            .toString(),
                        dockLevel: assignBidsVM.assignbidsList.value.data![index].dockLevel
                            .toString(),
                        note: assignBidsVM.assignbidsList.value.data![index].notes
                            .toString(),
                        onPress: (){
                          startTripVM.startTripApi(assignBidsVM.assignbidsList.value.data![index].loadId.toString());
                        //Get.to(HomeView(loadId:assignBidsVM.assignbidsList.value.data![index].loadId
                        // ));
